    ...
    public partial class Test : UserControl
    {
        ...
        private void btnTest_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                ...
                string[] pickTypes = {  "valid", "invalid","geçerli", "geçersiz" };
                string fileName = ofd.SafeFileName.ToLower();

                foreach (var f in format)
                    fileName = fileName.Replace(f, "");

                if (fileName == pickTypes[0] || fileName == pickTypes[2])
                    lblResult.Text = pickTypes[2].ToUpper();
                else if (fileName == pickTypes[1] || fileName == pickTypes[3])
